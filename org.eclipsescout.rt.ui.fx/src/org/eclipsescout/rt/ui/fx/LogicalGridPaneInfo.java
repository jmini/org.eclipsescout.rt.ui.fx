...
/**
 * Represents all informations for the {@link LogicalGridPane} to build the layout
 */
public class LogicalGridPaneInfo {

  LogicalGridData[/* node count */] gridDatas;
  Node[/* node count */] nodes;
  int cols;/* number of cells horizontally */
  int rows;/* number of cells vertically */
  double[/* column */][/* min,pref,max */] width;
  double[/* row */][/* min,pref,max */] height;
  double[/* column */] weightX;
  double[/* row */] weightY;
  private int m_hgap;
  private int m_vgap;

  LogicalGridPaneInfo(IFxEnvironment env, Node[] nodes, LogicalGridData[] cons, int hgap, int vgap) {
    this.nodes = nodes;
    this.m_hgap = hgap;
    this.m_vgap = vgap;
    ...
    this.width = new double[cols][3];
    this.height = new double[rows][3];
    this.weightX = new double[cols];
    this.weightY = new double[rows];
    initializeInfo(env, hgap, vgap);
  }

  private void initializeInfo(IFxEnvironment env, int hgap, int vgap) {
    int compCount = nodes.length;
    Bounds[] compSize = new Bounds[compCount];
    // cache component sizes and cleanup constraints
    for (int i = 0; i < compCount; i++) {
      Node comp = nodes[i];
      LogicalGridData cons = gridDatas[i];
      Bounds b = uiSizeInPixel(comp);
      double bWidth = b.getWidth();
      if (cons.widthHint > 0) {
        bWidth = cons.widthHint;
      }
      double bHeight = b.getHeight();
      if (cons.heightHint > 0) {
        bHeight = cons.heightHint;
      }
      compSize[i] = new BoundingBox(b.getMinX(), b.getMinY(), b.getMinZ(), bWidth, bHeight, b.getDepth());
      if (cons.gridx < 0) {
        cons.gridx = 0;
      }
      if (cons.gridy < 0) {
  ...
  private void initializeColumns(IFxEnvironment env, Bounds[] compSize, int hgap) {
    int compCount = compSize.length;
    double[] prefWidths = new double[cols];
    ...
    for (int i = 0; i < compCount; i++) {
      ...
      if (cons.gridw > 1) {
        ...
        if (cons.widthHint > 0) {
          ...
          distWidth = compSize[i].getWidth() - spanWidth - (hSpan - 1) * hgap;
        }
        else {
          distWidth = logicalWidthInPixel(env, cons) - spanWidth - (hSpan - 1) * hgap;
        }
        if (distWidth > 0) {
          double equalWidth = (distWidth + spanWidth) / hSpan;
          double remainder = (distWidth + spanWidth) % hSpan;
          int last = -1;
          for (int j = cons.gridx; j < cons.gridx + cons.gridw && j < cols; j++) {
            if (fixedWidths[j]) {
    ...
    for (int i = 0; i < cols; i++) {
      if (fixedWidths[i]) {
        width[i][FxLayoutUtility.MIN] = prefWidths[i];
        width[i][FxLayoutUtility.PREF] = prefWidths[i];
        width[i][FxLayoutUtility.MAX] = prefWidths[i];
      }
      else {
        width[i][FxLayoutUtility.MIN] = 0;// must be exactly 0!
        width[i][FxLayoutUtility.PREF] = prefWidths[i];
        width[i][FxLayoutUtility.MAX] = 10240;
      }
    }
    // averaged column weights, normalized so that sum of weights is equal to 1.0
    for (int i = 0; i < cols; i++) {
      if (fixedWidths[i]) {
        weightX[i] = 0;
      }
  ...
  private void initializeRows(IFxEnvironment env, Bounds[] compSize, int vgap) {
    int compCount = compSize.length;
    double[] prefHeights = new double[rows];
    ...
    for (int i = 0; i < compCount; i++) {
      ...
      if (cons.gridh > 1) {
        ...
        if (cons.heightHint > 0) {
          ...
          distHeight = compSize[i].getHeight() - spanHeight - (vspan - 1) * vgap;
        }
        else {
          distHeight = logicalHeightInPixel(env, cons) - spanHeight - (vspan - 1) * vgap;
        }
        if (distHeight > 0) {
          double equalHeight = (distHeight + spanHeight) / vspan;
          double remainder = (distHeight + spanHeight) % vspan;
          int last = -1;
          for (int j = cons.gridy; j < cons.gridy + cons.gridh && j < rows; j++) {
    ...
    for (int i = 0; i < rows; i++) {
      if (fixedHeights[i]) {
        height[i][FxLayoutUtility.MIN] = prefHeights[i];
        height[i][FxLayoutUtility.PREF] = prefHeights[i];
        height[i][FxLayoutUtility.MAX] = prefHeights[i];
      }
      else {
        height[i][FxLayoutUtility.MIN] = 0;// must be exactly 0!
        height[i][FxLayoutUtility.PREF] = prefHeights[i];
        height[i][FxLayoutUtility.MAX] = 10240;
      }
    }
   ...
   */
  Bounds[][] layoutCellBounds(double paramWidth, double paramHeight, Insets insets) {
    double[] w = layoutSizes(paramWidth - insets.getLeft() - insets.getRight() - Math.max(0, (cols - 1) * m_hgap), width, weightX);
    double[] h = layoutSizes(paramHeight - insets.getTop() - insets.getBottom() - Math.max(0, (rows - 1) * m_vgap), height, weightY);
    Bounds[][] cellBounds = new Bounds[rows][cols];
    double y = insets.getTop();
    for (int r = 0; r < cellBounds.length; r++) {
      double x = insets.getLeft();
      for (int c = 0; c < cellBounds[r].length; c++) {
        cellBounds[r][c] = new BoundingBox(x, y, w[c], h[r]);
        x += w[c];
        x += m_hgap;
      }
      y += h[r];
  ...
  private double[] layoutSizes(double targetSize, double[][] sizes, double[] weights) {
    double[] outSizes = new double[sizes.length];
    if (targetSize <= 0) {
      return new double[sizes.length];
    }
    int sumSize = 0;
    float[] tmpWeight = new float[weights.length];
    float sumWeight = 0;
    for (int i = 0; i < sizes.length; i++) {
      outSizes[i] = sizes[i][LogicalGridPane.PREF_SIZE];
      sumSize += outSizes[i];
       ...
       */
      if (tmpWeight[i] < LogicalGridPane.EPS) {
        if (sizes[i][LogicalGridPane.MAX_SIZE] > sizes[i][LogicalGridPane.MIN_SIZE]) {
          tmpWeight[i] = 1;
        }
        else {
          tmpWeight[i] = 0;
    ...
    double deltaInt = targetSize - sumSize;
    // expand or shrink
      ...
      if (deltaInt > 0) {
        ...
        while (deltaInt > 0 && hasTargets) {
          ...
          for (int i = 0; i < outSizes.length && deltaInt > 0; i++) {
            if (tmpWeight[i] > 0 && outSizes[i] < sizes[i][LogicalGridPane.MAX_SIZE]) {
              hasTargets = true;
        ...
        while (deltaInt < 0 && hasTargets) {
          hasTargets = false;
          for (int i = 0; i < outSizes.length && deltaInt < 0; i++) {
            if (tmpWeight[i] > 0 && outSizes[i] > sizes[i][LogicalGridPane.MIN_SIZE]) {
              hasTargets = true;
              accWeight[i] += tmpWeight[i];
              if (accWeight[i] > 0) {
                accWeight[i] -= 1;
  ...
  private static int logicalWidthInPixel(IFxEnvironment env, LogicalGridData cons) {
    int gridW = cons.gridw;
    return env.getFormColumnWidth() * gridW + env.getFormColumnGap() * Math.max(0, gridW - 1);
  }

  private static int logicalHeightInPixel(IFxEnvironment env, LogicalGridData cons) {
    int gridH = cons.gridh;
    return env.getFormRowHeight() * gridH + env.getFormRowGap() * Math.max(0, gridH - 1);
  }

  private static Bounds uiSizeInPixel(Node comp) {
    return comp.getLayoutBounds();
  }
}
